...
from utils_audio import load_model
...
#         self.vqgan = self.load_vqgan(args)
        self.config, self.vqgan, self.melgan, self.melception = self.load_vqgan(args)

        self.transformer = BidirectionalTransformer(n_embed=self.config.model.params.first_stage_config.params.n_embed,
                                                    codebook_size=args.num_codebook_vectors)
        if args.transformer_checkpoint_path is not None:
            self.load_checkpoint(path=args.transformer_checkpoint_path)
#             print("Transformer model checkpoint inputted from ",args.transformer_checkpoint_path)
...
#         model = model.eval()
#         return model
#     def load_config(args, display=False):
#         config = OmegaConf.load(args.config_path)
#         if display:
#             print(yaml.dump(OmegaConf.to_container(config)))
#         return config
    
#     def load_vqgan(config,args):
...
#         quant_z, indices, _ = self.vqgan.encode(x)
#        quant_z, _, ( _, _, indices) = self.vqgan.encode(x)
        quant_z, z_indices = self.vqgan.encode_to_z(x)
#         print("org:",z_indices.shape)
        z_indices = z_indices.view(quant_z.shape[0], -1)
        return quant_z, z_indices
    ...
    def sample(self, condition=None, num=1, T=10, temperature=1.0, mode="cosine"):
        ...
        if condition is None:
            indices = torch.zeros((num, N), device=device, dtype=torch.int)
        else:
            indices = torch.hstack((condition, torch.zeros((condition.shape[0], N-condition.shape[1]), device=device, dtype=torch.int)))
...
#             print("new_indices max", new_indices.max())
    ...
    def log_images(self, x, mode="cosine"):
        ...
        _, z_indices = self.encode_to_z(x)
#         print('log_image:', z_indices.max())

        # create a "half" sample
        z_start_indices = z_indices[:, :z_indices.shape[1]//2]
        half_index_sample = self.sample(condition=z_start_indices, mode=mode)
#         print(half_index_sample,half_index_sample.max())
#         print(half_index_sample.shape)
    ...
    def indices_to_image(self, indices, p1=5, p2=53):
#         ix_to_vectors = self.vqgan.codebook.embedding(indices).reshape(indices.shape[0], p1, p2, 256)
#         print(indices.min(),indices.max(), self.vqgan.first_stage_model.quantize.embedding.num_embeddings)
        ix_to_vectors = self.vqgan.first_stage_model.quantize.embedding(indices)
#         print(ix_to_vectors.shape)
        ix_to_vectors = ix_to_vectors.reshape(indices.shape[0], p1, p2, 256)
        ix_to_vectors = ix_to_vectors.permute(0, 3, 1, 2)
#         print(ix_to_vectors.shape)
        #image = self.vqgan.decode(ix_to_vectors)
        image = self.vqgan.first_stage_model.decode(ix_to_vectors)
        return image
    
#     def indices_to_image(self, index,  p1=5, p2=53, stage='first'):
# #         if stage == 'first':
# #             index = self.vqgan.first_stage_permuter(index, reverse=True)
# #         elif stage == 'cond':
...
#         bhwc = (index.shape[0], p1, p2, 256)
# #         quant_z = self.vqgan.first_stage_model.quantize.embedding(index.reshape(-1), shape=bhwc)
